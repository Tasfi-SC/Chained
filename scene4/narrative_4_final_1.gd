...
const dialog_lines : Array[String] = [
	"MC NAME:You're the voice.",
]

func _ready():
	dialog_index = 0
	process_current_line()
	character_sprite.position = Vector2(350, 550)

	dialog_ui.position = Vector2(10, 450)
...
func _input(event):
	if event.is_action_pressed("next_line"):
		if dialog_index < len(dialog_lines) -1:
			...
			get_tree().change_scene_to_file("res://scene4/narrative_4_final_2.tscn")
