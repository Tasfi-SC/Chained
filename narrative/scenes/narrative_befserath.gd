...
const dialog_lines: Array[String] = [
	"UNKNOWN: So she hasn't told you.",
	"MC NAME: ...who are you?",
	"UNKNOWN: You walk through doors she built.",
	"UNKNOWN: You take power she left behind.",
	"UNKNOWN: And you still think this is your path?",
...
func _input(event):
	if event.is_action_pressed("next_line"):
		if dialog_index < len(dialog_lines) -1:
			...
			get_tree().change_scene_to_file("res://scene2/zone_thealtar.tscn")
...
func process_current_line():
	...
	if speaker_change.has(speaker):
		...
		tween.tween_property(dialog_ui, "visible_characters", len(line_info["dialog_line"]), 2.0)
